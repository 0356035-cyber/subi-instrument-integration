...
- 风险提示面板（仅资源冲突）
- 受试者增删：工具栏添加；可复制已有受试者当前甘特流程；行首铅笔打开信息窗可编辑、删除，或把该受试者流程同步为项目模板
- 项目流程编辑：拖动/箭头调序，保存后按新顺序重建甘特图；支持粘贴整段流程拆成环节
- 云端保存/加载：经 `/api/gantt/state` 使用 Sub-I 工号登录同步（浏览器 localStorage 仍自动保存）
...
- 短环节识别：统一上下两层（上时间节点、下具体内容）+ 悬停提示 + 右侧「环节详情」面板
